...
function addKeyAndValue(arr, key, value) {
  return arr.reduce(function (accumulator, nextObj, index) {
...
function partition(arr, callback) {
  const newPartitionedArray = arr.reduce(
    function (accumulator, next) {
      if (callback(next)) {
        accumulator[0] = accumulator[0].concat(next);
      } else {
        accumulator[1] = accumulator[1].concat(next);
      }
      return accumulator;
    },
    [[], []]
  );
  return newPartitionedArray;
}
